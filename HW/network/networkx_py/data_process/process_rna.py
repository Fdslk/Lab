...
with open_workbook(input_file) as workbook:
	...
	for row_index in range(1, worksheet.nrows):
		...
		if  Class == interClass and Level == interLevel:
			if str(worksheet.cell_value(row_index, 4)) not in data:
				data.append(str(worksheet.cell_value(row_index, 4)))
				rna_no += 1
				row_list.append(rna_no)
				row_list.append(str(worksheet.cell_value(row_index, 4)))
